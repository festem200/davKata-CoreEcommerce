...
3. **Toda regla de adaptador nueva (`memory`/`json`/`postgres`) debe pasar la suite de contrato única** (`infrastructure/persistence/contract.test.ts`) antes de considerarse terminada. Si necesitas un caso que solo aplica a un adaptador, es una señal de que el contrato del puerto está mal definido — repórtalo, no lo escondas en un test aparte.
