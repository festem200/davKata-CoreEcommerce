...
const CATALOG = [{ id: "p4", name: "Power bank", category: "Tecnología", unitPriceCents: 3250, stock: 2 }];
...
function buildUseCase() {
  ...
  const useCase = new CheckoutUseCase(productRepository, orderRepository, engine, () => "order-fixed-id", () => new Date("2026-01-01T00:00:00Z"));
...
describe("CheckoutUseCase", () => {
  it("decrementa el stock y persiste la orden", async () => {
    ...
    const order = await useCase.execute({
      cartLines: [{ productId: "p4", quantity: 1 }],
      couponCode: null,
      idempotencyKey: "key-1",
    });

    expect(order.id).toBe("order-fixed-id");
    expect((await productRepository.findById("p4"))?.stock).toBe(1);
  });
  ...
  it("rechaza el checkout si el stock es insuficiente y no persiste ninguna orden", async () => {
    ...
    await expect(
      useCase.execute({
        cartLines: [{ productId: "p4", quantity: 3 }],
        couponCode: null,
  ...
  it("es idempotente: un reintento con la misma clave devuelve la orden ya creada sin volver a tocar el stock", async () => {
    const { useCase, productRepository } = buildUseCase();
    const input = { cartLines: [{ productId: "p4", quantity: 1 }], couponCode: null, idempotencyKey: "key-3" };

    const firstOrder = await useCase.execute(input);
    const secondOrder = await useCase.execute(input);

    expect(secondOrder).toEqual(firstOrder);
    expect((await productRepository.findById("p4"))?.stock).toBe(1); // no se decrementó dos veces
  });
});
